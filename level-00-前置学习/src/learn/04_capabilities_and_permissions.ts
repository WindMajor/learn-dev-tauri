 ...
 *
 * 【学习目标】
 *   1. 深入理解 V2 的 Capabilities 权限模型（替代 V1 的 allowlist）
 ...
 *   - V1 → V2 变化：从集中式 allowlist 改为分布式的 capabilities 文件
 *   - V2 的权限声明更灵活：可绑定到特定窗口、可限制 scope 范围
 *
 * 【V1 → V2 迁移重点】
 *   V1 tauri.conf.json 中的 allowlist 全部废弃！
...
// Tauri V2 特性：scope 是 V2 安全模型的核心，比 V1 的 allowlist scope 更灵活
 ...
 *
 * 2. V1 → V2 迁移关键：
 *    - 删除 tauri.conf.json 中的 allowlist（V2 不再解析）
